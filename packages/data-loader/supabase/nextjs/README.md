...
Below, we pass `where` to the `ServerDataLoader` component to filter the organizations by their name. We use the `textSearch` operator to search for the word `supabase` in the `name` column.
...
Below, we pass `select` to the `ServerDataLoader` component to select only the `id` and `name` columns.
...
The '_' wildcard is also supported - as in `'_'` (not an array). In this case, all the columns will be fetched.
