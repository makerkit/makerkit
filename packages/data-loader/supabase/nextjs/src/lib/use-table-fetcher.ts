...
export function useTableFetcher<
  ...
  Selection extends DataLoader.Query<DataLoader.ExtractDatabase<Client>, TableName> = DataLoader.StarOperator,
  Single extends boolean = false,
>(
  props: DataLoader.DataLoaderProps<Client, TableName, Selection, Single>,
): {
  data: DataType<Single, DataLoader.Data<DataLoader.ExtractDatabase<Client>, TableName, Selection>>;
  count: number;
  error: Error | null;
  isLoading: boolean;
} {
  const { data, error, isLoading } = useQuery<{
    ...
    return supabaseDataLoader<Client, TableName, Selection, Single>(
      props,
    );
  });
  ...
  if (!data) {
    return {
      data: (props.single ? undefined : []) as DataType<Single, DataLoader.Data<DataLoader.ExtractDatabase<Client>, TableName, Selection>>,
      count: 0,
      error,
      isLoading: true,
    };
