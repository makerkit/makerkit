...
test('Fetch a full table as an authed user', async (ctx) => {
  ...
  const response = renderHook(() => useSupabaseQuery({
    client,
    table: 'tasks',
  }));
...
test('Fetch a single item with ClientDataLoader', async (ctx) => {
      ...
      {props => {
        return (
          <>
            ...
            <span data-testid={'loading'} data-value={props.isLoading.toString()} />
          </>
        );
      }}
    </ClientDataLoader>,
  ...
  await vi.waitFor(() => {
    return ctx.expect(page.getByTestId('loading').dataset['value']?.trim()).toEqual('false');
  });
